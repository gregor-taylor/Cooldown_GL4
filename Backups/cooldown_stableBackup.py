...
#G Taylor Feb 2019
#
#Self contained GUI to cool down a Chase cryogenics fridge mounted in Giff-McMahon cryocooler to <1k
#
...
class MainWindow(QtWidgets.QMainWindow): #Master object
    def __init__(self, parent=None):
        ...
        #SIM900 Slots - Make sure devices are in these! Will be hard labelled on the SIM900
        self.ACBridgeSlot='5'
        self.ThermSlot1='8'
        self.ThermSlot2=''
        self.current_pump_temp=''
        #For loggin initially
    ...
    def confirm_devs(self, Keithley_add, SIM900_add): #Confirms the device choices and opens the devices for use under the Master object, then switches views to the cooldown view
        ...
        self.Keithley.write("SYSTEM:REMOTE") #needed to work on pi
...
class DevSetup(QtWidgets.QWidget): #Launch page where devices are defined from available list
    ...
    def populate_combo_box(self, widget, list): #Helper function to poulate a combobox
        for i in list:
            widget.addItem(i)

    def conf_devs(self, parent): #Checks for existence of devices
...
class Cooldown(QtWidgets.QWidget): #Cooldown page
    ...
    def update_GUI(self, temp_list): #Updates the GUI with temperatures and will initiate the stages as they are needed
        ...
        if self.Stage1 == True:
            if float(self.He_pump_temp.text()) > 45.0 and float(self.cold_head_temp.text()) < 4.2:
                self.CooldownThreadStage1.terminate()
                Master.Keithley.write("INST:NSEL 1")
                Master.Keithley.write("VOLT 0")
                Master.Keithley.write('CHAN:OUTP OFF')
        ...
        #this will monitor temps and trigger threads for stage 1/2 when neccessary
    
    def jump_to_stage_2(self):
...
class TempThread(QtCore.QThread): #Read the thermometers and returns the data to the GUI
    ...
    def run(self):
        while True:
            ...
            #NEED TO CHECK SLOTS FOR THERMOMETERS
